...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
            ...
            assign_input_dim=-1, item_num=0, args=None):
        '''
        Args:
        ...
        self.item_embeds = torch.nn.Embedding(item_num, embedding_dim*3)
        if args.pretrain:
            self.item_embeds.weight.data.copy_(torch.from_numpy(np.load('./data/item.embed.npy')))
    ...
    def get_item_embeds(self):
        self.item_embeds.weight.data = F.normalize(self.item_embeds.weight.data, p=2, dim=1)
        return self.item_embeds.weight

    def forward(self, x, adj, batch_num_nodes, **kwargs):
        # [bs x nodes x feature-size]
        x = x.expand(1, -1, -1)
        ...
        # loss 1
        # input : [batch_size x user index ]
        # embeds look up

        # out, _ = torch.max(embedding_tensor, dim=1)
        out = embedding_tensor
        out_all.append(out.squeeze())
        ...
        for i in range(self.num_pooling):
            ...
            out_all.append(tensor.squeeze())
            assign_out = self.assign_tensor
            assign_all.append(assign_out.squeeze())

        # out_all list of [batch_size x node x embedding_size]
        # assign_all list of [batch_size x node x node1]
        for i, o in enumerate(out_all):
            out_all[i] = F.normalize(o, p=2, dim=1)

        return out_all, assign_all

    def pred_loss(self, logits):
        bs, samples = logits[0].size()
        x = torch.LongTensor([0]*bs)
        # the first column is 1
        loss = super(SoftPoolingGcnEncoder, self).loss(logits[0], x) + super(SoftPoolingGcnEncoder, self).loss(logits[1], x)
        return loss

    def negative_sample(self, range_n, num):
        return [random.random()*range_n for i in range(num)]

    def pred_item_based_logits(self, emb_all, assign_all, label, args):
        # label = (user indexes, item indexes)
        # for each user sample one positive item and some negative sample
        bs = len(label[0])
        c0_embeds = emb_all[0]
        # [bs x neg_num+1 x embedding_size]
        # print (label[0].size())
        user_pos_ids = torch.LongTensor(label[0])
        uids = torch.LongTensor([[id]*(self.neg_num+1) for id in label[0]])
        user_embeds = c0_embeds[uids]
        # item ids
        pos_ids = label[1].view(-1,1)
        neg_ids = torch.LongTensor(self.negative_sample(self.item_num, self.neg_num)).expand(bs, -1)
        item_ids = torch.cat([pos_ids, neg_ids], dim = 1)
        item_embeds = self.item_embeds(Variable(torch.LongTensor(item_ids)))

        # [bs x neg_num+1 x 1]
        logits_0 = torch.matmul(torch.unsqueeze(user_embeds, 2), torch.unsqueeze(item_embeds, 3))

        c1_assign = assign_all[0]
        c1_embeds = emb_all[1]
        # [bs x 1 x level2_num]
        c1_assign = c1_assign[user_pos_ids]
        # [bs x 1 x level2_num]
        if args.use_top:
            c1_one_hot = torch.nn.functional.gumbel_softmax(c1_assign.squeeze(), tau=1.0, hard=True, eps=1e-10)
            # [bs x self. x embedding_size]
            c_embeds = torch.matmul(c1_one_hot, c1_embeds)
        else:
            c_embeds = torch.matmul(c1_assign.squeeze(), c1_embeds)

        c_embeds = torch.unsqueeze(c_embeds, 1).expand(-1, self.neg_num+1, -1)
        
        # bs x users x 1 
        logits_1 = torch.matmul(torch.unsqueeze(c_embeds, 2), torch.unsqueeze(item_embeds, 3))
