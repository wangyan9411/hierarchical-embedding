...
import cross_val
# import no_community as encoders
import encoders
...
def train(dataset, model, user_feature, adjacency, args, same_feat=True, val_dataset=None, test_dataset=None, writer=None,
        mask_nodes = True):
    ...
    test_epochs = []
    val_accs = []
    adj = Variable(torch.Tensor(adjacency), requires_grad=False)
    h0 = Variable(torch.Tensor(user_feature), requires_grad=False)
    assign_input = h0
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, data in enumerate(dataset):
            model.zero_grad()
            train_data = (data['uid'], data['iid'])
            batch_num_nodes = None
            # batch_num_nodes = data['num_nodes'].int().numpy() if mask_nodes else None

            # forward function invoking
            # logits = model(h0, adj, batch_num_nodes, assign_x=assign_input)
            # assign_x = h0
            embed_all, assign_all = model(h0, adj, batch_num_nodes)
            if not args.method == 'soft-assign' or not args.linkpred:
                loss = model.pred_loss(model.pred_item_based_logits(embed_all, assign_all, train_data, args))
            else:
                loss = model.pred_loss(model.pred_item_based_logits(embed_all, assign_all, train_data, args))
            loss.backward()
            ...
            iter += 1
            avg_loss += loss
            print('Avg loss: ', avg_loss/(batch_idx + 1))
            '''
            ...
            '''
        # save
        item_embeds = model.get_item_embeds()
        save_numpy(embed_all, assign_all, item_embeds, epoch, args)
...
def save_numpy_0(user_embeds, assigns, item_embeds, epoch, args):
    u_embeds = user_embeds[0].detach().numpy()
    item_embeds = item_embeds.detach().numpy()
    print ('user embedding matrix size ', u_embeds.shape)
    print ('item embedding matrix size ', item_embeds.shape)

    res_dir = './results_only_user/'

    np.save(res_dir + 'user_' + str(epoch) + '.npy', u_embeds)
    np.save(res_dir + 'item_' + str(epoch) + '.npy', item_embeds)

def save_numpy(user_embeds, assigns, item_embeds, epoch, args):
    assert len(user_embeds) == 2
    assert len(assigns) == 1
    u_embeds = user_embeds[0].detach().numpy()
    c_embeds = user_embeds[1].detach().numpy()
    assign = assigns[0].detach().numpy()
    item_embeds = item_embeds.detach().numpy()
    print ('user embedding matrix size ', u_embeds.shape)
    print ('community embedding matrix size ', c_embeds.shape)
    print ('assignment matrix size ', assign.shape)
    print ('item embedding matrix size ', item_embeds.shape)

    res_dir = './results'
    if args.user_feature_with_behav:
        res_dir += '_with_behav'
    if args.pretrain:
        res_dir += '_pretrain'
    if not args.use_top:
    ...
    np.save(res_dir + 'user_' + str(epoch) + '.npy', u_embeds)
    np.save(res_dir + 'community_' + str(epoch) + '.npy', c_embeds)
    np.save(res_dir + 'assign_' + str(epoch) + '.npy', assign)
    np.save(res_dir + 'item_' + str(epoch) + '.npy', item_embeds)

class userdata(torch.utils.data.Dataset):
    def __init__(self, fp):
        self.train_data = np.load(fp)
        print ('load train data')
        print (self.train_data.shape, self.train_data[0][0], self.train_data[1][0])

    def __getitem__(self, index):
        return {'uid': self.train_data[0][index], 'iid': self.train_data[1][index]}

    def __len__(self):
        return self.train_data.shape[1]

def prepare_data(fp, args):
    dataset = userdata(fp)
...
def load_graph(args):
    filedir = '/home/wy/diffpool/data/'
    if args.user_feature_with_behav:
        h0 = np.load(filedir + 'user_feature_with_behav.npy') 
    else:
        h0 = np.load(filedir + 'user_feature.npy') 
    adj = np.load(filedir + 'adj.npy') 
    max_num_nodes = 10000
    input_dim = h0.shape[1]
    assign_input_dim = h0.shape[1]
    return max_num_nodes, input_dim, assign_input_dim, h0, adj 

def syn_community1v2(args, writer=None, export_graphs=False):
    filedir = '/home/wy/diffpool/data/'
    train_dataset = prepare_data(filedir + 'train.npy', args)
    item_num = 4000
    max_num_nodes, input_dim, assign_input_dim, user_feature, adjacency = load_graph(args)
    if args.method == 'soft-assign':
        ...
        model = encoders.SoftPoolingGcnEncoder(
                ...
                bn=args.bn, linkpred=args.linkpred, assign_input_dim=assign_input_dim, item_num = item_num, args = args)

    train(train_dataset, model, user_feature, adjacency, args, writer=writer)
...
def arg_parse():
    ...
    parser.add_argument('--with_behav', dest='user_feature_with_behav', 
            default=True,
            help='Whether link prediction side objective is used')
    parser.add_argument('--use_top', dest='use_top', 
            default=False,
            help='Whether link prediction side objective is used')
    parser.add_argument('--use_pretrain', dest='pretrain', 
            default=True,
            help='Whether link prediction side objective is used')
